...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], petCareCenterId, petcarecenter, numfields, mapItem;

        petcarecenter = row['petcarecenter'];
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) petCareCenterId = rslt.id;
                            
                            const mapPropKey = petcarecenter.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "PcId", value: parseInt(petCareCenterId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdatePetCareCenter(data, dropDownOptions);
        ...
        if (changes.length > 0 && changes.indexOf('Logo') > -1) {
            data = petcarecenter.find((x) => x.key === 'Logo');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "PcId", value: parseInt(petCareCenterId) },
                    { key: "PetCareCenterLogo", value: newImageId }
                ];
                rslt = await Support.AddOrUpdatePetCareCenter(data);
                if (!rslt.status) return;

                let newValues = petcarecenter.find((x) => x.key === 'Logo').value;
                newValues = { ...newValues, DocId: newImageId };
                row['petcarecenter'].find((x) => x.key === 'Logo').value = newValues;
                                // Update Back for next tracking purpose
                UpdateBackUp('petcarecenter');
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'petcarecenter');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        window.AlertPopup("success", "PetCareCenter is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
